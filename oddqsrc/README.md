...
Building SNOVA requires a C compiler, `make` and the OpenSSL library (for AES). The SNOVA parameters are set in `snova_params.h`. The SNOVA parameters can also be changed by the command line parameters of the `make` command, e.g.
```
...
3. Use `make OPT=AVX2` for a version using AVX2 vectorization instructions (odd prime $q$ and $l=4$ only). The plain-C optimized version is faster on some compilers.
...
The performance of the sign on other compilers can be made to be close to that of gcc 15.1.1 by using explicit AVX2 vectorization instructions. Vectorization is supported for odd prime $q$ and $l=4$.
Benchmark results for the vectorized `OPT=AVX2` version of SNOVA_24_5_23_4 are:

| Compiler | version |   Genkey  |     Sign   |  Verify  |
|----------|---------|-----------|------------|----------|
|   gcc    |  15.2.1 |   456,141 |    849,425 |  356,909 |
