...
static void hash_combined(uint8_t *hash_out, const uint8_t *m, size_t mlen, const uint8_t *pk_seed,
                          const uint8_t *salt) {
	shake_t state;
	shake256_init(&state);
...
int SNOVA_NAMESPACE(sign)(const expanded_SK *skx, uint8_t *sig, const uint8_t *digest, const size_t len_digest,
	...
	do {
		...
		gf_t Left[SNOVA_o * SNOVA_alpha * SNOVA_v * SNOVA_l2] = {0};
		gf_t Right[SNOVA_o * SNOVA_alpha * SNOVA_v * SNOVA_l2] = {0};
...
int SNOVA_NAMESPACE(verify)(const expanded_PK *pkx, const uint8_t *sig, const uint8_t *digest, const size_t len_digest) {
	...
	// Expand pkx further
	gf_t P[SNOVA_o * SNOVA_n * SNOVA_n * SNOVA_l2];
	...
	for (size_t idx = 0; idx < SNOVA_o * SNOVA_n * SNOVA_n * SNOVA_l2; idx++) {
		P[idx] = pkx->P[idx];
	}
	 ...
	 */
	gf_t Left[SNOVA_o * SNOVA_alpha * SNOVA_n * SNOVA_l2] = {0};
	gf_t Right[SNOVA_o * SNOVA_alpha * SNOVA_n * SNOVA_l2] = {0};
